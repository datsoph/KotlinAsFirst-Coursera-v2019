...
fun ageDescription(age: Int): String =
    if (age % 100 != 11 && age % 10 == 1) "$age год" else if (age % 100 !in 10..19 && age % 10 in 2..4) "$age года" else "$age лет"
...
fun timeForHalfWay(
    ...
    return when {
        t1 * v1 >= length / 2 -> length / 2 / v1
        t1 * v1 + t2 * v2 >= length / 2 -> t1 + (length / 2 - t1 * v1) / v2
        else -> t1 + t2 + (length / 2 - t1 * v1 - t2 * v2) / v3
    }
}
...
fun whichRookThreatens(
    ...
    return when {
        kingX == rookX1 && kingY == rookY1 || kingX == rookX2 && kingY == rookY2 -> 418
...
fun rookOrBishopThreatens(
    ...
    return when {
        kingX == rookX && kingY == rookY || kingX == bishopX && kingY == bishopY -> 418
